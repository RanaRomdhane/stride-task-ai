...
export const UserManagementPanel = ({ isAdmin, currentUserDepartment }: UserManagementPanelProps) => {
  ...
  const canEditUser = (user: UserWithRole) => {
    ...
    // Sub-admins can edit users in their department only
    return user.department === currentUserDepartment && user.role !== 'admin';
  };

  // Filter users to only show those in the current department for sub-admins
  const displayUsers = isAdmin ? users : users.filter(user => user.department === currentUserDepartment);
